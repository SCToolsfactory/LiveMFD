...
namespace MFDlib.Protocol
{
  ...
  class UdpMessenger
  {
    ...
    public static bool CheckIP( string ipAddr )
    {
    ...
    public UdpMessenger( string remoteHost, uint remotePort )
    {
      m_host = remoteHost;
      m_port = remotePort;
      ...
      if ( IPAddress.TryParse( remoteHost, out IPAddress lAddr ) ) {
        m_ipAddress = lAddr;
        m_endpoint = new IPEndPoint( m_ipAddress, (int)m_port );
    ...
    /// Send a message
    /// </summary>
    /// <param name="message">A VJCommand message</param>
    /// <returns>True if it succeeds</returns>
    public bool SendMsg( vjMapper.VjOutput.VJCommand message )
    {
      ...
      byte[] buffer = ascii.GetBytes( message.JString );
